import numpy as np
from hh_distance_tc import hh_distance_tc
from vacancy_check import vacancy_check
def tract_distance_tc(g, tot_tracts, BTS_df, ACS_df, checks):
    # Initialize tracking parameters
    tot_vtrips = 0 #np.zeros(8808)
    ...
    for hh in range(hh_cnt):
        [checks, vtrp_i] = hh_distance_tc(ACS_tract_df, BTS_tract_df, ug_scale, nan_check, checks)
        tot_vtrips = tot_vtrips + int(vtrp_i)

    return [checks, nan_check, tot_vtrips]
