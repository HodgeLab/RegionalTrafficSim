...
def hh_details_set(ACS_tract_df, BTS_tract_df, checks):
    ...
    hh_room_ind = [i+1 for i in range(len(hh_room_pct))]
    ...
    av_veh_pct = [ACS_tract_df["pct_0_veh"].values[0], ACS_tract_df["pct_1_veh"].values[0], ACS_tract_df["pct_2_veh"].values[0], ACS_tract_df["pct_3p_veh"].values[0]]
    av_veh_ind = [i for i in range(len(av_veh_pct))]
    av_veh_i = random.choices(av_veh_ind, weights = av_veh_pct, k=1)[0]
    #NOTE: meant to be 3+ av vehicles and 9+ hh rooms, we have elected the minimum

    # 50/50 chance of having 3 or 4 vehicles when av_veh_i[0] = 3
    if av_veh_i == 3:
