            ...
            if EV_chghome > 0:
                ...
                if day == 366:
                    ...
                    if surp_energy > 0:
                        ...
                        surp_energy = 0
                    surp_charge = np.zeros(24)
            if abs(sum(ann_hourly_energy) - sum(ann_hourly_home_energy) - surp_energy - sum(surp_charge)) > 1:
                print("tract number, tract, hh, day: ", g, " ", tot_tracts[g], " ", i, " ",day)
...
wb1 = xlsxwriter.Workbook('TEST_Tract_Energies_P' + pt_num + 'test.xlsx')
...
for g in range(tract_range[1]):
    w1.write(0, g+1, tot_tracts[g])
    w1.write_column(1, g+1, geo_energy[g])
    w2.write(0, g+1, tot_tracts[g])
    w2.write_column(1, g+1, geo_h_energy[g])
wb1.close()
